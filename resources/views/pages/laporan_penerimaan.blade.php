<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex gap-8 flex-wrap sm:rounded-lg">
            ...
            <h1 class="font-semibold text-slate-800">Laporan Penjualan Tabung Gas</h1>
    ...
    <script>
        ...
        const jenisGas = {!! json_encode($jenis_gas) !!}
        const label = [];
        const dataset = [];
        const temp = [];

        jenisGas.forEach(element => {
            dataset.push(store[element.jenis_gas].length);
            label.push(element.jenis_gas);
        });

        // let ctx = document.getElementById('pie-chart').getContext('2d');
        const selectMonth = () => {
            const selectedValue = document.getElementById('selected-month').value;
            window.location.href = '/laporan/penerimaan/' + selectedValue;
        }
